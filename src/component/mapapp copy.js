...
class mapApp extends React.Component {
    ...
    componentDidMount() {
        ...
        $(document).on('click', '#asyncresult > div', (e) => {
            ...
            $('#searchInput input').val(txt).focus();
            $('#asyncresult').empty();
    ...
    handleonKeyUp(e){
        if(e.keyCode >= 46 & e.keyCode <= 96 || e.keyCode === 8 || e.keyCode === 32){
    ...
    getData(){
        ...
        if (this.refs.search.value !== ''){
            ...
            $.getJSON('http://m3311.pages.labranet.jamk.fi/web-visualisointi/map.json', 'features', function(data) {
                $.each(data.features, (index, element) => {
                    ///console.log(element.properties.name);
                    let name = element.properties.name;
                    let address = element.properties.address;
                    if(name.search(expression) !== -1 || address.search(expression) !== -1){
                        ///console.log(element.properties.name);
